...
public class GameOfWhalesAndroid : GameOfWhales{
	...
	protected override void _Initialize()
	{
        try
        {
            ...
            if (settings == null)
            {
                Debug.LogError("[GameOfWhales] Initialization error. Settings file is missing. Check 'Assets/Resources' folder");
                return;
            }
    ...
    protected override void _Purchase(string product, string currency, double price, string transactionID, string receipt, bool verify) 
    {
        try
        {
            ...
            {
                gameofwhales.Call("purchase", product, currency, price.ToString("G", CultureInfo.InvariantCulture), transactionID, receipt, verify.ToString());
            }
        }
        catch (Exception e)
        {
    ...
    protected override void _Purchase(string product, string currency, double price) 
    {
        try
        {
            ...
            {
                gameofwhales.Call("purchase", product, currency, price.ToString("G", CultureInfo.InvariantCulture));
            }
        }
        catch (Exception e)
        {
    ...
    protected override void _InAppPurchased(string sku, double price, string currency, string transactionID, string receipt) 
    {
        try
        {
            ...
            {
                gameofwhales.Call("inAppPurchased", sku, price.ToString("G", CultureInfo.InvariantCulture), currency, transactionID, receipt);
            }
        }
        catch (Exception e)
        {
    ...
    protected override Experiment _GetCurrentExperiment()
    {
        try
        {
            if (checkInstance("GetCurrentExperiment"))
            {
                string result = gameofwhales.Call<string>("GetCurrentExperiment");

                if (result != null && result.Length > 1)
                {
                    Experiment experiment = CreateExperiment(result);
                    return experiment;
                }
            }
        }
    ...
    protected override void _CallStringResult(string result)
    {
        try
        {
            if (checkInstance("CallStringResult"))
            {
                gameofwhales.Call("receiveResult", result);
            }
        }
        catch (Exception e)
        {
